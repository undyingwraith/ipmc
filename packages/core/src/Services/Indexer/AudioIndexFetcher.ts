...
import { IFileInfo, IIpfsService, IIpfsServiceSymbol, ILibrary, ILogService, ILogServiceSymbol, IAudioMetaData } from 'ipmc-interfaces';
...
@injectable()
export class AudioIndexFetcher implements IIndexFetcher<IAudioMetaData[]> {
	constructor(
	 ...
	 * @inheritdoc
	 */
	public async fetchIndex(options: IFetchOptions<IAudioMetaData[]>): Promise<IAudioMetaData[]> {
	...
	public async extractMusicMetaData(entry: IFileInfo, parentId: string, old: IAudioMetaData[] | undefined, signal: AbortSignal): Promise<IAudioMetaData[]> {
